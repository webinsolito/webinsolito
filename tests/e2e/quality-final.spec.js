...
test('merged duplicates point users to the stronger app',async({page})=>{
 const merges={ 'damage-log':'accident-kit','car-docs':'docpocket','home-docs':'docpocket','booking-lite':'appointment','receipt-box':'receipt-pocket','bollo-check':'autobuddy','revisione-memo':'autobuddy','tyre-memo':'autobuddy' };
...
test('catalog has 192 active apps and eight documented merges',async({request})=>{
 const d=await (await request.get('/apps.json')).json();
 const active=d.apps.filter(a=>['MVP','BETA','STABLE'].includes(a.status)&&a.path);
 expect(active).toHaveLength(192);
 expect(d.apps.filter(a=>a.status==='MERGED')).toHaveLength(8);
});
