...
docs:[["Organizzo documenti","docpocket"],["Ritrovo screenshot","screensort"],["Salvo una garanzia","warranty-pocket"],["Documenti viaggio","travel-docs"]],
...
let C,A=[],ALL=[];const appById=id=>ALL.find(a=>a.id===id&&["MVP","BETA","STABLE"].includes(a.status)&&a.path);
function draw(query=""){const live=A.filter(a=>["MVP","BETA","STABLE"].includes(a.status)&&a.path);const ranked=query.trim()&&window.WebinsolitoSearch?WebinsolitoSearch.search(query,live,[C],60).filter(x=>x.type==="app").map(x=>x.app):live;const seen=new Set(),visible=ranked.filter(a=>a&&!seen.has(a.id)&&(seen.add(a.id),true));apps.innerHTML=visible.length?visible.map(a=>'<a class="app" href="../'+a.path+'"><img src="../'+a.icon+'" alt="" loading="lazy" decoding="async"><h3>'+esc(a.name)+'</h3><p>'+esc(a.description)+'</p></a>').join(""):'<div class="empty">Nessuno strumento trovato. Prova a descrivere il problema con altre parole.</div>';apps.setAttribute("aria-label",visible.length+" strumenti disponibili")}
fetch("../apps.json",{cache:"no-store"}).then(r=>r.json()).then(x=>{ALL=x.apps;C=x.categories.find(c=>c.id===catId);A=ALL.filter(a=>a.category===catId);catIcon.src="../"+C.icon;catIcon.decoding="async";catTitle.textContent=C.name;search.setAttribute("aria-label","Cerca strumenti in "+C.name);search.setAttribute("aria-controls","apps");search.setAttribute("type","search");apps.setAttribute("aria-live","polite");intentGrid.innerHTML=(intents[catId]||[]).map(([label,id])=>{const a=appById(id);return a?'<a class="intent" href="../'+a.path+'">'+esc(label)+'</a>':""}).join("");draw();search.oninput=()=>draw(search.value)}).catch(()=>apps.innerHTML='<div class="empty">Categoria non disponibile.</div>');
})();